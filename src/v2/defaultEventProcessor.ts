...
export const defaultEventProcessor: EventProcessor = (entry) => {
  ...
  if (entry.entryType === 'resource') {
    ...
    const resource = metadata.resource && typeof metadata.resource === 'object' && metadata.resource;
    const resourceType = resource && typeof resource.type === 'string' && resource.type;
    const statusCode = resource && typeof resource.status_code === 'number' && resource.status_code;
    ...
    commonName = `${entry.entryType}${entry.name &&
        entry.name !== 'unknown' &&
        entry.name.length > 0 &&
...
export const lotusEventProcessor: EventProcessor = (entry) => {
  ...
  if (processedEntry.entryType === 'mark' || processedEntry.entryType === 'measure') {
